...
class FlightOrbit3DCard extends HTMLElement {
  ...
  _onMapLoad() {
    ...
    try {
      ...
      try {
        ...
        if (!String(layerError?.message || layerError).includes("symbol-height")) throw layerError;
        this._config = { ...this._config, actual_altitude: false };
        if (this._map.getLayer("flight-labels")) this._map.removeLayer("flight-labels");
        if (this._map.getLayer("aircraft-symbols")) this._map.removeLayer("aircraft-symbols");
        if (this._map.getLayer("selected-aircraft-ring")) this._map.removeLayer("selected-aircraft-ring");
        if (this._map.getLayer("flight-tracks")) this._map.removeLayer("flight-tracks");
        if (this._map.getSource("flight-points")) this._map.removeSource("flight-points");
        if (this._map.getSource("flight-trails")) this._map.removeSource("flight-trails");
        this._installFlightLayers();
      }
      this._mapReady = true;
      this._applyData();
  ...
  _installAircraftImages() {
    ...
    for (const [name, image] of Object.entries(icons)) {
      if (!this._map.hasImage(name)) this._map.addImage(name, image, { pixelRatio: 2 });
    }
  }

  _canvasImageData(width, height, draw) {
    const canvas = document.createElement("canvas");
    canvas.width = width;
    canvas.height = height;
    const context = canvas.getContext("2d");
    draw(context);
    // MapLibre addImage does not accept a raw canvas element.
    return context.getImageData(0, 0, width, height);
  }

  _makePlaneIcon(color, glow) {
    return this._canvasImageData(96, 96, (context) => {
  ...
  _makeRingIcon() {
    return this._canvasImageData(112, 112, (context) => {
  ...
  _installFlightLayers() {
    ...
    if (this._config.actual_altitude !== false) {
      commonSymbol["symbol-height-offset"] = ["get", "heightMeters"];
      commonSymbol["symbol-height-anchor"] = "absolute";
    }
    ...
    this._map.addLayer({
      ...
      layout: {
        "symbol-placement": "point",
        ...(this._config.actual_altitude !== false ? {
